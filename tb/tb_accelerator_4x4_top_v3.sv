...
module tb_accelerator_4x4_top_v3;
    ...
    initial begin : tests
        ...
        fd=$fopen("vectors/v3_vectors.txt", "r");
        ...
        for (integer id=0; id<vectors; id++) begin
            for (integer n=0; n<16; n++) begin
                rc=$fscanf(fd, "%d", va[id][n]);
                if (rc != 1 || va[id][n] < -128 || va[id][n] > 127) $fatal(1, "bad A vector");
            end
            for (integer n=0; n<16; n++) begin
                rc=$fscanf(fd, "%d", vb[id][n]);
                if (rc != 1 || vb[id][n] < -128 || vb[id][n] > 127) $fatal(1, "bad B vector");
            end
            for (integer n=0; n<16; n++) begin
                rc=$fscanf(fd, "%d", vc[id][n]);
